...
class TracchildticketsModule(Component):
    ...
    _stats_provider = ExtensionOption('childtickets', 'stats_provider',
    ...
    def _link_parent(self, req, filename, data):
        # this is shamelessly stollen from MasterTickets and modified for one field only
        ...
        if filename in ["report_view.html", "query_results.html", "ticket.html", "query.html"]:
            ...
            if 'fields' in data and isinstance(data['fields'], list):
                for field in data['fields']:
                    if field['name'] == self.field_name and data['ticket'][self.field_name]:
                        field['rendered'] = self._link_ticket(req, data['ticket'][self.field_name])
            # For query_results.html and query.html
            if 'groups' in data and isinstance(data['groups'], list):
                for group, tickets in data['groups']:
                    for ticket in tickets:
                        if self.field_name in ticket:
                            ticket[self.field_name] = self._link_ticket(req, ticket[self.field_name])
                        ...
                        if 'cell_groups' in row and isinstance(row['cell_groups'], list):
                            for cells in row['cell_groups']:
                                for cell in cells:
                                    ...
                                    if cell.get('header', {}).get('col') == self.field_name:
                                        cell['value'] = self._link_ticket(req, cell['value'])
        return

    def _link_ticket(self, req, ticketid):
        # little safeguard
